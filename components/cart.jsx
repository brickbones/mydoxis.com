...
export default function Cart({ helpers }) {
  const { locale } = useRouter()
  const count = helpers.cartCount()

  return (
    count > 0 && (
      <div className='z-50 md:fixed md:top-4 md:right-4'>
        <Link href='/cart'>
          <a className='flex items-center text-xl font-bold text-white md:text-gray-300 md:bg-gray-900 rounded-sm md:rounded-md md:p-2.5 md:text-base'>
            <svg
              className='block w-8 h-8 mr-1 fill-current md:w-6 md:h-6'
        ...
        <div
          className={
            'bg-orange-500 hidden md:block w-72 h-72 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 border-4 border-orange-500 bg-opacity-25 rounded-full mini-circle-00'
          }
          style={{ zIndex: '-1', pointerEvents: 'none' }}
        />
        <div
          className={
            'bg-orange-500 hidden md:block w-72 h-72 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 border-4 border-orange-500 bg-opacity-25 rounded-full mini-circle-01'
          }
          style={{ zIndex: '-1', pointerEvents: 'none' }}
        />
      </div>
